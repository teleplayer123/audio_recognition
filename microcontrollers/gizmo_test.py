...
def get_spectrogram(data):
    ...
    #spect[0] = 0
    spect = np.log(spect + 1e-7)
    spect = spect[1:(fft_size//2)-1]
    n_max = np.max(spect)
    n_min = np.min(spect)
    #set min and max threshold for graph data
    n_max = min(n_max, thresh_max)
    n_min = max(n_min, thresh_min)
    data = (spect - n_min) * (51.0 / (n_max - n_min))
